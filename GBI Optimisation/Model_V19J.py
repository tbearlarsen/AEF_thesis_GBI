...
n_paths = 3  # Assuming you want to simulate all 9999 paths
    ...
    while True:
        ...
        for idx, g in enumerate(goals):
            ...
            if horizon == 1:
                ...
                goal_ratios = [goal_data_raw[f"GOAL {goals[i]}"].values[0] for i in range(num_goals)]
        ...
        print("------------------------------\n")
    # Create subfolder for each path (optional but cleaner)
    path_folder = os.path.join(output_folder, f"path_{path_id}")
...
pd.concat(master_asset_log, ignore_index=True).to_csv(os.path.join(output_folder, "master_asset_returns_log.csv"), index=False)
pd.concat(master_final_wealth, ignore_index=True).to_csv(os.path.join(output_folder, "master_final_wealth_by_year.csv"), index=False)
pd.concat(master_goal_weights, ignore_index=True).to_csv(os.path.join(output_folder, "master_goal_weights_by_year.csv"), index=False)
pd.concat(master_aggregate_portfolio, ignore_index=True).to_csv(os.path.join(output_folder, "master_aggregate_portfolio_by_year.csv"), index=False)
