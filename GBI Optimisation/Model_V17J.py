        ...
        for idx, g in enumerate(goals):
            ...
            if horizon == 1:
                ...
                if redeem:
                    ...
                    goal_reached[g].append(True)
                if not redeem:
                    goal_reached[g].append(False)
...
#%%
# After simulation, count successes and failures
goal_distribution_summary = []

for g in ["A", "B", "C", "D"]:
    successes = sum(goal_reached[g])
    failures = len(goal_reached[g]) - successes
    goal_distribution_summary.append({
        "Goal": g,
        "Successes": successes,
        "Failures": failures
    })

df_goal_distribution = pd.DataFrame(goal_distribution_summary)

print("\n--- Goal Success/Failure Distribution ---")
print(df_goal_distribution)

# Optionally save
df_goal_distribution.to_csv(os.path.join(output_folder, "goal_success_distribution.csv"), index=False)
