...
for path_id in range(n_paths):
        ...
        for idx, g in enumerate(goals):
            ...
            if horizon == 1:
                ...
                if redeem:
                    ...
                    goal_reached[g].append({"path_id": path_id, "success": True})
                if not redeem:
                    goal_reached[g].append({"path_id": path_id, "success": False})
    ...
    pd.DataFrame(asset_level_log).to_csv(os.path.join(path_folder, "asset_returns_log.csv"), index=False)

    # Export final wealth
    pd.DataFrame.from_dict(final_wealth_tracker, orient='index', columns=['Final Wealth']).to_csv(
        os.path.join(path_folder, "final_wealth_by_year.csv")
    )

    # Export goal weights
    pd.DataFrame(goal_weights_tracker).to_csv(
        os.path.join(path_folder, "goal_weights_by_year.csv"), index=False
    )

    # Export aggregate portfolio
    pd.DataFrame(aggregate_portfolio_tracker).to_csv(
        os.path.join(path_folder, "aggregate_portfolio_by_year.csv"), index=False
    )
    ...
    for goal_letter, matrix_dict in zip(
        ...
        df_all.to_csv(os.path.join(path_folder, f"optimal_weights_goal_{goal_letter}.csv"), index=False)

    path_results[path_id] = {
        "final_wealth": final_wealth_tracker.copy(),
        "goal_weights": goal_weights_tracker.copy(),
        "aggregate_portfolio": aggregate_portfolio_tracker.copy()
    }
...
df_goal_distribution = pd.DataFrame(records)

# Quick summary if needed
summary = df_goal_distribution.groupby("Goal")["Success"].agg(["sum", "count"])
...
# Save detailed data and summary if you want
df_goal_distribution.to_csv(os.path.join(output_folder, "goal_success_detailed.csv"), index=False)
