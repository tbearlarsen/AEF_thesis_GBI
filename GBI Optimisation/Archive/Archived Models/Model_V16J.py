...
#%% md
# # Second Loop: Used right now to rerun everything but without having to do the optimisation (grabs the first loop's optimal weights)
#%%
print("\n===== BEGIN STATIC OPTIMIZATION LOOP =====\n")
# Trackers for the second (static) loop
final_wealth_tracker_second = {}
asset_level_log_second = []
goal_weights_tracker_second = []
aggregate_portfolio_tracker_second = []

aktiesparekonto_used_total = 0

for loop_year in range(start_year, end_year + 1):
    print(f"[STATIC LOOP] Year {loop_year}")

    salary = df_salary.loc[Profile, str(loop_year)]
    prev_year = loop_year - 1

    if loop_year == start_year:
        prev_wealth = 0
    else:
        prev_wealth = final_wealth_tracker[prev_year]

    pool = salary + prev_wealth

    # Retrieve stored goal weights for this year
    goal_weights_row = next(item for item in goal_weights_tracker if item["Year"] == loop_year)
    normalized_goal_alloc = np.array([goal_weights_row[f"Goal {g} (%)"] for g in goals])

    goal_weights_tracker_second.append({
    "Year": loop_year,
    **{f"Goal {goals[i]} (%)": normalized_goal_alloc[i] for i in range(len(goals))}
})

    # Retrieve stored aggregate portfolio for this year
    df_agg = pd.DataFrame([
        row for row in aggregate_portfolio_tracker if row["Year"] == loop_year
    ])
    asset_names = df_agg["Asset"].tolist()
    weights = df_agg["Weight"].to_numpy()

    # Calculate invested amounts
    df_agg["Invested Amt (DKK)"] = weights * pool

    for i in range(len(asset_names)):
        aggregate_portfolio_tracker_second.append({
            "Year": loop_year,
            "Asset": asset_names[i],
            "Weight": weights[i],
            "Allocation (%)": df_agg.iloc[i]["Invested Amt (DKK)"] / pool * 100,
            "Invested Amt (DKK)": df_agg.iloc[i]["Invested Amt (DKK)"]
        })

    # Apply ASK logic as before
    equity_rows = []
    non_equity_rows = []
    ...
    total_equity = sum(r["Invested"] for r in equity_rows)
    remaining_ask_cap = max(0, aktiesparekonto_cap - aktiesparekonto_used_total)
    ask_fraction = min(1, remaining_ask_cap / total_equity) if total_equity > 0 else 0

    for row in equity_rows:
        ask = row["Invested"] * ask_fraction
        row["ASK (DKK)"] = ask
        row["Normal (DKK)"] = row["Invested"] - ask
        aktiesparekonto_used_total += ask

    account_allocations = equity_rows + non_equity_rows

    # Tax logic from main loop (can be refactored if reused)
    share_income_normal = 0
    asset_level_this_year = []
    ...
    for row in account_allocations:
        ...
        normal = row["Normal (DKK)"]
        expected_return = df_returns.loc[asset, str(loop_year)]
        expected_return_inflation_adj = expected_return - inflation_rate

        if ask > 0:
            gain = ask * expected_return_inflation_adj
            tax = 0.17 * gain
            asset_level_this_year.append({
                "Year": loop_year,
                "Account": "ASK",
                "Asset": asset,
                "Invested": ask,
                "Return (Adj)": expected_return_inflation_adj,
                "Tax": tax,
                "Net Gain": gain - tax,
                "End Value": ask + gain - tax
            })
        ...
        if normal > 0:
            gain = normal * expected_return_inflation_adj
            share_income_normal += gain
            asset_level_this_year.append({
                "Year": loop_year,
                "Account": "NORMAL",
    ...
    # Tax distribution for NORMAL accounts
    cap = 67500
    if share_income_normal <= cap:
        tax_normal = 0.27 * share_income_normal
    else:
    ...
    norm_rows = [r for r in asset_level_this_year if r["Account"] == "NORMAL"]
    total_normal_gain = sum(r["Gross Gain"] for r in norm_rows)
    for r in norm_rows:
        share = r["Gross Gain"] / total_normal_gain if total_normal_gain else 0
        tax = share * tax_normal
        r["Tax"] = tax
        r["Net Gain"] = r["Gross Gain"] - tax
        r["End Value"] = r["Invested"] + r["Net Gain"]

    end_value_sum = sum(r["End Value"] for r in asset_level_this_year)
    # -- V9 Logic in Static Loop -- #
    goal_data_raw = get_goal_data(master_excel_path, plan=Profile)
    goal_horizons = goal_data_raw.loc["Time Horizon"].astype(int)
    goal_years = start_year + goal_horizons
    adjusted_horizons = goal_years - loop_year
    goal_data_raw.loc["Time Horizon"] = adjusted_horizons
    ...
    for i, goal in enumerate(goals):
        if adjusted_horizons[f"GOAL {goal}"] == 1:
            ...
            net_util, fut_loss, phi_details = net_utility_change_from_goal_funding(
                ...
                future_goals,
                future_weights,
                df_returns.loc[df_agg["Asset"], str(loop_year)].to_numpy() - inflation_rate,
                covariances
            )

            if net_util > 0:
                amount_taken = min(goal_required, amount_alloc_based)
                print(f"[STATIC DECISION] ✅ Paying Goal {goal} | ΔUtility: {net_util:.3f}")
                end_value_sum -= amount_taken
            else:
                amount_taken = 0
                print(f"[STATIC DECISION] ❌ Skipping Goal {goal} | ΔUtility: {net_util:.3f}")
    ...
    # Annotate current year's static asset log
    for row in asset_level_this_year:
        for key, value in goal_payout_info.items():
            row[key] = value

    final_wealth_tracker_second[loop_year] = end_value_sum
    # Optional: Append asset_level_this_year to a new static log
    asset_level_log_second += asset_level_this_year

    print(f"→ [STATIC] Year {loop_year}: Final Wealth = {end_value_sum:.2f}")
